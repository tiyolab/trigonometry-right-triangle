...
public class FindAngleATest extends AndroidTestCase {
    ...
    /**
     * test ruleAngleA2b, tan <A known
     */
    public void testRuleAngleA2c_1(){
        Map<String, ItemDescriptionDialog> listItemsDescription = new HashMap<String, ItemDescriptionDialog>();
        ArrayList<ItemDescriptionDialog> listItemsQuestion = new ArrayList<ItemDescriptionDialog>();
        ...
        ItemDescriptionDialog id = new ItemDescriptionDialog(ConstantHelper.TAN_A,"tan <A",null);
        id.setTNumber(t1);

        listItemsDescription.put(ConstantHelper.TAN_A, id);
        ...
        Assert.assertEquals("tan <A", asw.get(0).getLeftOperand().getTitle());
        Assert.assertEquals(id.getTNumber().getValue(), asw.get(0).getRightOperand().get(0).getTNumber().getValue());

        Assert.assertEquals("<A", asw.get(1).getLeftOperand().getTitle());
        Assert.assertEquals("atan(tan <A)", asw.get(1).getRightOperand().get(0).getTitle());

        Assert.assertEquals("<A", asw.get(2).getLeftOperand().getTitle());
        Assert.assertEquals(AngleConverterHelper.radToDeg(Math.atan(0.5)), asw.get(2).getRightOperand().get(0).getTNumber().getIntegerNumber().getIntegerValue());
    }

    /**
     * test ruleAngleA2b, a and b known
     */
    public void testRuleAngleA2c_2(){
        Map<String, ItemDescriptionDialog> listItemsDescription = new HashMap<String, ItemDescriptionDialog>();
        ArrayList<ItemDescriptionDialog> listItemsQuestion = new ArrayList<ItemDescriptionDialog>();

        TNumber t1 = new TNumber();
        t1.setType(TNumber.INTEGER_NUMBER);
        t1.setIntegerNumber(new FractionNumber(1, 0, 9, 5));
        ...
        ItemDescriptionDialog id2 = new ItemDescriptionDialog(ConstantHelper.BC_LENGTH,"BC",null);
        id2.setTNumber(t2);

        listItemsDescription.put(ConstantHelper.AB_LENGTH, id1);
        listItemsDescription.put(ConstantHelper.BC_LENGTH, id2);
        ...
        Assert.assertEquals("tan <A", asw.get(0).getLeftOperand().getTitle());
        Assert.assertEquals("BC // AB", asw.get(0).getRightOperand().get(0).getTitle());

        Assert.assertEquals("tan <A", asw.get(1).getLeftOperand().getTitle());
        Assert.assertEquals(id2.getTNumber().getValue(), asw.get(1).getRightOperand().get(0).getTNumber().getValue());
        Assert.assertEquals(" // ", asw.get(1).getRightOperand().get(1).getTitle());
        Assert.assertEquals(id1.getTNumber().getValue(), asw.get(1).getRightOperand().get(2).getTNumber().getValue());

        Assert.assertEquals("tan <A", asw.get(2).getLeftOperand().getTitle());
        Assert.assertEquals(MathHelper.division(id2.getTNumber(), id1.getTNumber()).getValue(), asw.get(2).getRightOperand().get(0).getTNumber().getValue());

        Assert.assertEquals("tan <A", asw.get(3).getLeftOperand().getTitle());
        Assert.assertEquals(MathHelper.division(id2.getTNumber(), id1.getTNumber()).getValue(), asw.get(3).getRightOperand().get(0).getTNumber().getValue());

        Assert.assertEquals("<A", asw.get(4).getLeftOperand().getTitle());
        Assert.assertEquals("atan(tan <A)", asw.get(4).getRightOperand().get(0).getTitle());

        Assert.assertEquals("<A", asw.get(5).getLeftOperand().getTitle());
        Assert.assertEquals(AngleConverterHelper.radToDeg(Math.atan(MathHelper.division(id2.getTNumber(), id1.getTNumber()).getValue())), asw.get(5).getRightOperand().get(0).getTNumber().getValue());
    }
}
